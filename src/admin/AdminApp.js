...
import { formatLogTimestamp, getRangeFromPreset } from "./lib/date";
import {
...
const OverviewKpis = ({ range }) => {
  ...
  const visitsTooltipShort = __(
    "Anonymous sessions aggregated over the selected period.",
    "lean-stats",
  );
  const visitsTooltip =
...
const TimeseriesChart = ({ range }) => {
  ...
  const formatPageViewsLabel = (value) =>
    sprintf(
      _n("%s page view", "%s page views", value, "lean-stats"),
